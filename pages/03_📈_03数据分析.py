...
MODEL_NAME = os.getenv("MODEL_CODER", "deepseek-v3-0324")
    ...
    if analyze_btn:
        ...
        with st.status(
            ...
            with get_openai_callback() as cb:
                html_content, report_path, context_data = run_agent_pipeline(
                    target_dfs, user_query, API_KEY, API_BASE
                )

                tokens = cb.total_tokens
                # [FIX] 原来硬编码 8000；改为按输入输出字符数动态估算（字符数 / 3 ≈ token 数）
                # 分子：query + 所有表的列名摘要（避免序列化整张表）+ 估算的 output
                if tokens == 0:
                    col_summary = " ".join(
                    ...
                    estimated_output_chars = 3000  # 多表分析平均输出量
